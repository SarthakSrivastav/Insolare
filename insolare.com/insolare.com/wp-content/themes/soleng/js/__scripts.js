 ...
 (function() {
             ...
             case "object":
                 if (mixed_val == null) return "N;";
                     ...
                     for (var i = 0; i < mixed_val.length; i++) {
                         ...
                         if (ser) map.push(soleng_serialize(idxobj.idx) + ser);
                     }
                     ...
                     for (var prop in mixed_val) {
                         ...
                         if (ser) props.push(soleng_serialize(prop) + ser);
                     }
     ...
     window.soleng_get_date_difference = function(dt1) {
         ...
         } else var a2 = soleng_parse_date(dt2);
     ...
     window.soleng_del_cookie = function(name, path, domain) {
         ...
         if (soleng_get_cookie(name)) document.cookie = name + '=' + ((path) ? ';path=' + path : '') + ((domain) ? ';domain=' + domain : '') + ';expires=Thu, 01-Jan-1970 00:00:01 GMT';
     };
     ...
     window.soleng_debug_object = function(obj) {
         ...
         if (obj !== null && (typeof(obj) == 'object' || typeof(obj) == 'array')) {
             for (var prop in obj) {
                 ...
                 if (level < recursive && (typeof(obj[prop]) == 'object' || typeof(obj[prop]) == 'array') && obj[prop] != obj) dispStr += addStr + prop + '=' + soleng_debug_object(obj[prop], recursive, showMethods, level + 1);
                 else {
                     try {
                         curStr = "" + obj[prop];
                     } catch (e) {
     ...
     window.dco === undefined && (window.dco = function(s, r) {
         console.log(soleng_debug_object(s, r));
     });
     window.dal === undefined && (window.dal = function(s) {
         if (SOLENG_STORAGE['user_logged_in']) alert(s);
     });
     window.dao === undefined && (window.dao = function(s, r) {
         if (SOLENG_STORAGE['user_logged_in']) alert(soleng_debug_object(s, r));
     });
     window.ddl === undefined && (window.ddl = function(s, c) {
         soleng_debug_log(s, c);
     });
     window.ddo === undefined && (window.ddo = function(s, r, c) {
         soleng_debug_log(soleng_debug_object(s, r), c);
     });
 })();
 jQuery(document).ready(function() {
     ...
     function soleng_ready_actions() {
         ...
         if (document.documentElement.className.indexOf(SOLENG_STORAGE['site_scheme']) == -1) document.documentElement.className += ' ' + SOLENG_STORAGE['site_scheme'];
         ...
         jQuery('.menu_mobile,.sc_layouts_menu_dir_vertical').on('click', 'li a, li a .open_child_menu', function(e) {
             ...
             if (!jQuery(this).hasClass('open_child_menu') && jQuery(this).parents('.menu_mobile').length > 0 && soleng_is_local_link($a.attr('href'))) jQuery('.menu_mobile_close').trigger('click');
         ...
         jQuery('.nav-links-more a').on('click', function(e) {
             ...
             if (panel.length == 0) {
                 jQuery.get(location.href, {
                     ...
                     soleng_loadmore_add_items(jQuery('.content .posts_container').eq(0), jQuery(response).find('.content .posts_container > article,' + '.content .posts_container > div[class*="column-"],' + '.content .posts_container > .masonry_item'));
                 });
             } else {
                 jQuery.post(SOLENG_STORAGE['ajax_url'], {
                     ...
                     try {
                         ...
                         rez = {
                             error: SOLENG_STORAGE['strings']['ajax_error']
                         };
             ...
             function soleng_loadmore_add_items(container, items) {
                 ...
                 else SOLENG_STORAGE['load_more_link_busy'] = false;
                 jQuery(window).trigger('scroll');
             }
             e.preventDefault();
             return false;
     ...
     function soleng_stretch_sidemenu() {
         ...
         var toc_items_height = jQuery(window).height() - soleng_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - toc_items.length;
     ...
     jQuery(document).on('action.toc_menu_item_active', function() {
         ...
         var th = toc_items.eq(0).height(),
             ...
             menu_side_height = jQuery(window).height() - soleng_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - jQuery('.menu_side_wrap .sc_layouts_logo + .toc_menu_item').outerHeight();
     ...
     function soleng_check_layout() {
         ...
         if (SOLENG_STORAGE['mobile_device'] || soleng_browser_is_mobile()) jQuery('body').addClass('mobile_device');
     }
         ...
         if (sb.length > 0) {
             if (content.css('float') == 'none') {
                 ...
                 if (sb_height < content_height && scroll_offset + top_panel_fixed_height > content_top) {
                     ...
                     if (scroll_dir == 0) scroll_dir = SOLENG_STORAGE['scroll_dir_last'];
         ...
         if (SOLENG_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
             if (window.mejs) {
                 ...
                 cont.find('audio:not(.inited),video:not(.inited)').each(function() {
                     ...
                     if (jQuery(this).addClass('inited').parents('.mejs-mediaelement').length == 0 && (SOLENG_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode') && !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist') && jQuery(this).parents('.elementor-background-video-container').length == 0))) {
     ...
     function soleng_tabs_ajax_content_loader(panel, page, oldPanel) {
         ...
         } else panel.find('> *').addClass('soleng_tab_content_remove');
     ...
     function soleng_comments_validate(form) {
         ...
         var comments_args = {
             error_message_text: SOLENG_STORAGE['strings']['error_global'],
             error_message_show: true,
             error_message_time: 4000,
             error_message_class: 'soleng_messagebox soleng_messagebox_style_error',
             ...
             rules: [{
                 ...
                 min_length: {
                     value: 1,
                     message: SOLENG_STORAGE['strings']['text_empty']
                 },
                 max_length: {
                     value: SOLENG_STORAGE['comment_maxlength'],
                     message: SOLENG_STORAGE['strings']['text_long']
                 }
             }]
         };
         if (form.find('.comments_author input[aria-required="true"]').length > 0) {
             comments_args.rules.push({
                 ...
                 min_length: {
                     value: 1,
                     message: SOLENG_STORAGE['strings']['name_empty']
                 },
                 max_length: {
                     value: 60,
                     message: SOLENG_STORAGE['strings']['name_long']
                 }
             });
         }
         if (form.find('.comments_email input[aria-required="true"]').length > 0) {
             comments_args.rules.push({
                 ...
                 min_length: {
                     value: 1,
                     message: SOLENG_STORAGE['strings']['email_empty']
                 },
                 max_length: {
                     value: 60,
                     message: SOLENG_STORAGE['strings']['email_long']
                 },
                 mask: {
                     value: SOLENG_STORAGE['email_mask'],
                     message: SOLENG_STORAGE['strings']['email_not_valid']
                 }
             });
         }
 ...
 jQuery(document).on('action.init_hidden_elements', function(e, cont) {
     ...
     if (SOLENG_STORAGE['button_hover'] && SOLENG_STORAGE['button_hover'] != 'default') {
         jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]),\ .theme_button:not([class*="sc_button_hover_"]),\ .sc_button:not([class*="sc_button_simple"]):not([class*="sc_button_hover_"]),\ .sc_form_field button:not([class*="sc_button_hover_"]),\ .post_item .more-link:not([class*="sc_button_hover_"]),\ .trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]),\ .soleng_tabs .soleng_tabs_titles li a:not([class*="sc_button_hover_"]),\ .hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]),\ .mptt-navigation-tabs li a:not([class*="sc_button_hover_style_"]),\ .edd_download_purchase_form .button:not([class*="sc_button_hover_style_"]),\ .edd-submit.button:not([class*="sc_button_hover_style_"]),\ .widget_edd_cart_widget .edd_checkout a:not([class*="sc_button_hover_style_"]),\ .woocommerce #respond input#submit:not([class*="sc_button_hover_"]),\ .woocommerce .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"]),\ .woocommerce-page .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"]),\ #buddypress a.button:not([class*="sc_button_hover_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_' + SOLENG_STORAGE['button_hover']);
         ...
         jQuery('.mejs-controls button,\ .mfp-close,\ .sc_button_bg_image,\ .hover_shop_buttons a,\ button.pswp__button,\ .woocommerce-orders-table__cell-order-actions .button,\ .sc_layouts_row_type_narrow .sc_button\ ').removeClass('sc_button_hover_' + SOLENG_STORAGE['button_hover']);
     }
 });
 (function() {
     "use strict";
